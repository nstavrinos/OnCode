...
app.post('/SignUp', (req,res)=>{
    ...
    .exec((error , user) => {
        ...
        if(user){
            res.send({message: "Username alreafy exists. Choose another one."});
        }
        else{
                ...
                if (err) {
                   console.log(err);
                    ...
                    if(data.includes(repolink) || data.includes(repolink.slice(0,-4))){                           
                            git.pull(req.body.username, req.body.branch)
                            .then(()=>{
                            ...
                            git.init()
                            .addRemote(req.body.username, repolink)
                            .pull(req.body.username, req.body.branch)
                            .then(()=>{
                                console.log('Pulling Repo finished\n');
           ...
           console.log(err);
            ...
            if(data.includes( req.body.repolink) || data.includes( req.body.repolink.slice(0,-4))){
                ...
                .exec((error,user) => {
                    ...
                    .push(req.body.username, req.body.branch,["--set-upstream","--force"])
                    .then(()=>{
                ...
                .exec((error , user) => {
                    git.init()
                    .addRemote(req.body.username, req.body.repolink)
            ...
            if (fs.lstatSync(fullPath).isDirectory()) {
                fs.readdirSync(fullPath).forEach(child => {
                        ...
                        if (fs.lstatSync(childrenPath).isDirectory()) {      
                            projects.push({name:child,parent:file});
                        }  
                    });
            }
        });
